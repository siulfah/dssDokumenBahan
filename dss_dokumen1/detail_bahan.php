<!--<?php 
//session_start();
//header("location: index.php");
?>-->
<div class="panel panel-warning">
  <div class="panel-heading">
    <p class="panel-title" ><strong>Data Bahan</strong></p>
  </div>

  <div class="panel-body">

    <?php
          include('app/config.php');
          $id_bahan = $_GET['id_bahan'];
          $query = mysql_query("SELECT * FROM bahan WHERE id_bahan='$id_bahan'");
          $cetak = mysql_fetch_array($query);
        ?>
        <table class="table" border="0">
            <tr>
              <td align="right" width="40%">Nama bahan</td>
              <td width="1%">:</td>
              <td align="left" width="50%"><?=$cetak['nama_bahan'];?></td>
            </tr>
            <tr>
              <td align="right">Sumber bahan baku</td>
              <td width="1%">:</td>
              <td align="left"><?=$cetak['sumber_bahan'];?></td>
            </tr>
            <tr>
              <td align="right">Sumber bahan tambahan</td>
              <td width="1%">:</td>
              <td align="justify"><?=$cetak['sbt'];?></td>
            </tr>
            <tr>
              <td align="right">Titik kritis bahan</td>
              <td width="1%">:</td>
              <td align="left"><?=$cetak['titik_kritis'];?></td>
            </tr>
            <tr>
              <td align="right">Dokumen pendukung</td>
              <td width="1%">:</td>
              <td align="left"><?=$cetak['dokumen'];?></td>
            </tr>
            <tr>
              <td align="right">Deskripsi bahan</td>
              <td width="1%">:</td>
              <td align="left"><?=$cetak['deskripsi'];?></td>
            </tr>
            <tr>
              <td align="right">Gambar</td>
              <td width="1%">:</td>
              <td align="left"><?=$cetak['gambar'];?></td>
            </tr>
            <tr>
              <td align="right"></td>
              <td width="1%"></td>
              <td align="left"><a href="index.php?page=lihat_bahan"><button type="reset" class="btn btn-warning" value="Kembali">Kembali</button>&nbsp;&nbsp;&nbsp;</td>
            </tr>
        </table>

</div>